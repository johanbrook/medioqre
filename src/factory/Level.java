	...
	private void parseNestedConfigFiles(JSONObject configFile) {
		if (configFile == null) {
			throw new IllegalArgumentException(
					"Input config file can't be null");
		}
		...
		while (it.hasNext()) {
			...
			try {
				...
				if (configKey.endsWith("Config")
					...
					this.configFiles.put(configKey,
							configFile.getString(configKey));
					configValue = ResourceLoader.parseJSONFromPath(configFile
							.getString(configKey));
					log("Loading " + configKey + " file from "
							+ configFile.get(configKey) + " ...");
				} else {
