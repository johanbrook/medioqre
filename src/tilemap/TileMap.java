 ...
 * 
 * It stores a tiletype in a two dimensional int array which is matched against
 * its current tileSheet at rendertime.
 * 
...
public class TileMap implements GLRenderableObject {
	...
	/**
	 * Creates a tilemap with the given amount of rows, columns, the given
	 * tilesheet and pixels.
	 * 
	 * @param rows
	 *            The number of rows.
	 * @param columns
	...
	public TileMap(int rows, int columns, TileSheet tileSheet, int[] pixels) {
		this.tiles = pixels == null ? new int[columns][rows] : PixelCastingTool
				.get2dTileMatrixFromPixelArray(columns, rows, pixels);
	...
	public void setViewPortSize(Size size) {
		if (this.tileViewPort == null)
			this.tileViewPort = new Rectangle(0, 0, 0, 0);
	...
	public void setTileSize(Size size) {
		...
		this.tileRenderRect = new Rectangle(0, 0, this.tileSize.getWidth(),
	 ...
	 * Get collidables.
	 * 
	 * @return A two dimensional array of booleans representing what tiles are
	 *         collidable
	 */
	public boolean[][] getCollidables() {
		...
		for (int y = 0; y < collidables.length; y++) {
			for (int x = 0; x < collidables[y].length; x++) {
				collidables[x][y] = this.tileSheet.getTile(this.tiles[x][y])
						.isCollidable();
			}
		}
		return collidables;
		...
		for (int x = 0; x < this.tiles.length; x++) {
			for (int y = 0; y < this.tiles[x].length; y++) {
				...
				if (this.tileSheet.getTile(this.tiles[x][y]) == null) {
					System.out
							.println("Trying to render a tile that is null!: "
									+ this.tiles[x][y]);
					return;
				}

				this.tileSheet.getTile(this.tiles[x][y]).render(tileRenderRect,
						target, canvas, 0);
			}
		}
	}
